...
public:
  ...
  void pub_global_path(vector<vector<ScanPose*>*> &relc_bl_buf, const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pub_relc, vector<int> &ids)
  {
  ...
  void pub_globalmap(vector<vector<Keyframe*>*> &relc_submaps, vector<int> &ids, const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pub)
  {
    pcl::PointCloud<pcl::PointXYZI> pl;
    pub_pl_func(pl, pub);
    pcl::PointXYZI pp;
...
public:
  ...
  {
    string premap;
    ros2_param<string>(n, "General/previous_map", premap, "");
  ...
  void previous_map_read(vector<STDescManager*> &std_managers, vector<vector<ScanPose*>*> &multimap_scanPoses, vector<vector<Keyframe*>*> &multimap_keyframes, ConfigSetting &config_setting, PGO_Edges &edges, rclcpp::Node::SharedPtr &n, vector<string> &fnames, vector<double> &juds, string &savepath, int win_size)
  {
    ...
    ros2_param<int>(n, "Loop/acsize", acsize, 10);
    ros2_param<int>(n, "Loop/mgsize", mgsize, 5);
...
public:
  ...
  int motion_init(vector<pcl::PointCloud<PointType>::Ptr> &pl_origs, vector<deque<sensor_msgs::msg::Imu::SharedPtr>> &vec_imus, vector<double> &beg_times, Eigen::MatrixXd *hess, LidarFactor &voxhess, vector<IMUST> &x_buf, unordered_map<VOXEL_LOC, OctoTree*> &surf_map, unordered_map<VOXEL_LOC, OctoTree*> &surf_map_slide, vector<PVecPtr> &pvec_buf, int win_size, vector<vector<SlideWindow*>> &sws, IMUST &x_curr, deque<IMU_PRE*> &imu_pre_buf, IMUST &extrin_para)
  {
    ...
    double t0 = ros2_now_seconds(g_node);
    ...
    double t1 = ros2_now_seconds(g_node);
    printf("init time: %lf\n", t1 - t0);
...
public:
  ...
  VOXEL_SLAM(rclcpp::Node::SharedPtr &n)
  {
    ...
    ros2_param<string>(n, "General/lid_topic", lid_topic, "/livox/lidar");
    ros2_param<string>(n, "General/imu_topic", imu_topic, "/livox/imu");
    ros2_param<string>(n, "General/bagname", bagname, "site3_handheld_4");
    ros2_param<string>(n, "General/save_path", savepath, "");
    ros2_param<int>(n, "General/lidar_type", feat.lidar_type, 0);
    ros2_param<double>(n, "General/blind", feat.blind, 0.1);
    ros2_param<int>(n, "General/point_filter_num", feat.point_filter_num, 3);
    ros2_param<vector<double>>(n, "General/extrinsic_tran", vecT, vector<double>());
    ros2_param<vector<double>>(n, "General/extrinsic_rota", vecR, vector<double>());
    ros2_param<int>(n, "General/is_save_map", is_save_map, 0);

    sub_imu = n->create_subscription<sensor_msgs::msg::Imu>(imu_topic, rclcpp::SensorDataQoS().keep_last(80000), imu_handler);
    if(feat.lidar_type == LIVOX)
    {
#ifdef VOEXLSLAM_HAS_LIVOX
      sub_livox = n->create_subscription<livox_ros_driver2::msg::CustomMsg>(lid_topic, rclcpp::SensorDataQoS().keep_last(1000), pcl_handler<livox_ros_driver2::msg::CustomMsg::SharedPtr>);
#else
      RCLCPP_ERROR(n->get_logger(), "lidar_type is LIVOX, but livox_ros_driver2 was not found when building voexlslam.");
      exit(1);
#endif
    }
    else
    {
      sub_pcl = n->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, rclcpp::SensorDataQoS().keep_last(1000), pcl_handler<sensor_msgs::msg::PointCloud2::SharedPtr>);
    }
    odom_ekf.imu_topic = imu_topic;

    ros2_param<double>(n, "Odometry/cov_gyr", cov_gyr, 0.1);
    ros2_param<double>(n, "Odometry/cov_acc", cov_acc, 0.1);
    ros2_param<double>(n, "Odometry/rdw_gyr", rand_walk_gyr, 1e-4);
    ros2_param<double>(n, "Odometry/rdw_acc", rand_walk_acc, 1e-4);
    ros2_param<double>(n, "Odometry/down_size", down_size, 0.1);
    ros2_param<double>(n, "Odometry/dept_err", dept_err, 0.02);
    ros2_param<double>(n, "Odometry/beam_err", beam_err, 0.05);
    ros2_param<double>(n, "Odometry/voxel_size", voxel_size, 1);
    ros2_param<double>(n, "Odometry/min_eigen_value", min_eigen_value, 0.0025);
    ros2_param<int>(n, "Odometry/degrade_bound", degrade_bound, 10);
    ros2_param<int>(n, "Odometry/point_notime", point_notime, 0);
    ...
    ros2_param<int>(n, "LocalBA/win_size", win_size, 10);
    ros2_param<int>(n, "LocalBA/max_layer", max_layer, 2);
    ros2_param<double>(n, "LocalBA/cov_gyr", cov_gyr, 0.1);
    ros2_param<double>(n, "LocalBA/cov_acc", cov_acc, 0.1);
    ros2_param<double>(n, "LocalBA/rdw_gyr", rand_walk_gyr, 1e-4);
    ros2_param<double>(n, "LocalBA/rdw_acc", rand_walk_acc, 1e-4);
    ros2_param<int>(n, "LocalBA/min_ba_point", min_ba_point, 20);
    ros2_param<vector<double>>(n, "LocalBA/plane_eigen_value_thre", plane_eigen_value_thre, vector<double>({1, 1, 1, 1}));
    ros2_param<double>(n, "LocalBA/imu_coef", imu_coef, 1e-4);
    ros2_param<int>(n, "LocalBA/thread_num", thread_num, 5);
  ...
  void lio_state_estimation_kdtree(PVecPtr pptr)
  {
    ...
    double tt1 = ros2_now_seconds(g_node);
    for(pointVar pv: *pptr)
    {
      pv.pnt = x_curr.R * pv.pnt + x_curr.p;
      PointType ap;
    ...
    kd_map.setInputCloud(pl_tree);
    double tt2 = ros2_now_seconds(g_node);
  }
  ...
  void loop_update()
  {
    ...
    loop_detect = 0;
    double t2 = ros2_now_seconds(g_node);
  ...
  void thd_odometry_localmapping(rclcpp::Node::SharedPtr &n)
  {
    ...
    while(rclcpp::ok())
    {
      ...
      ros2_param<bool>(n, "finish", is_finish, false);
      if(is_finish)
      {
        break;
      }
      ...
      double t_end = ros2_now_seconds(g_node);
  ...
  void thd_loop_closure(rclcpp::Node::SharedPtr &n)
  {
    ...
    int isHighFly = 0;
    ros2_param<double>(n, "Loop/jud_default", jud_default, 0.45);
    ros2_param<double>(n, "Loop/icp_eigval", icp_eigval, 14);
    ros2_param<double>(n, "Loop/ratio_drift", ratio_drift, 0.05);
    ros2_param<int>(n, "Loop/curr_halt", curr_halt, 10);
    ros2_param<int>(n, "Loop/prev_halt", prev_halt, 30);
    ros2_param<int>(n, "Loop/isHighFly", isHighFly, 0);
  ...
  {
    ros2_param<double>(n, "GBA/voxel_size", gba_voxel_size, 1.0);
    ros2_param<double>(n, "GBA/min_eigen_value", gba_min_eigen_value, 0.01);
    ros2_param<vector<double>>(n, "GBA/eigen_value_array", gba_eigen_value_array, vector<double>());
    for(double &iter: gba_eigen_value_array) iter = 1.0 / iter;
    int total_max_iter = 1;
    ros2_param<int>(n, "GBA/total_max_iter", total_max_iter, 1);
    ...
    while(rclcpp::ok())
    {
      ...
      double tg1 = ros2_now_seconds(g_node);
...
int main(int argc, char **argv)
{
  ...
  g_node = rclcpp::Node::make_shared("cmn_voxel");
  ...
  thread thread_loop(&VOXEL_SLAM::thd_loop_closure, &vs, ref(g_node));
  thread thread_gba(&VOXEL_SLAM::thd_globalmapping, &vs, ref(g_node));
