...
template <typename T>
void pub_pl_func(T &pl, const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pub)
{
...
void imu_handler(const sensor_msgs::msg::Imu::SharedPtr msg_in)
{
  ...
  // double tc = ros2_stamp_seconds(*msg);
  // if(tc > t0 && tc < t1)
  //   msg->linear_acceleration.z = -9.7;

  mBuf.lock();
  imu_last_time = ros2_stamp_seconds(*msg);
...
bool sync_packages(pcl::PointCloud<PointType>::Ptr &pl_ptr, deque<sensor_msgs::msg::Imu::SharedPtr> &imus, IMUEKF &p_imu)
{
  ...
  mBuf.lock();
  double imu_time = ros2_stamp_seconds(*imu_buf.front());
  while((!imu_buf.empty()) && (imu_time < p_imu.pcl_end_time)) 
  {
    imu_time = ros2_stamp_seconds(*imu_buf.front());
...
void icp_check(pcl::PointCloud<PointType> &pl_src, pcl::PointCloud<PointType> &pl_tar, const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pub_src, const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pub_tar, pair<Eigen::Vector3d, Eigen::Matrix3d> &loop_transform, IMUST &xx)
{
